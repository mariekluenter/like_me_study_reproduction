---
title: "Reproduction of data processing script for rating passive study"
date: last-modified
freeze: true
cache: true
format:
...
```{r}
files <- list.files(path = here("R_scripts", "scripts_and_data_original", "Data_Preprocessing_original", "RatingsStudy_passive", "full_passives_data"), pattern = "\\.csv$", full.names = T)

raw_data <- sapply(files, read_csv, simplify=FALSE, col_types = cols(list = col_character())) %>% 
  bind_rows(.id = "id") %>%
  mutate(condition = "passive")

coding = read_csv(here("R_scripts", "scripts_and_data_original", "Data_Preprocessing_original", "RatingsStudy_passive", "coding.csv")) #load file with regular/reverse coding info
...
write_csv(data,here("R_scripts", "scripts_reproduction" ,"Data_Preprocessing_reproduction", "reproduced_data", "rating_study_passive.csv"))
```
