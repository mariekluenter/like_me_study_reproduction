---
title: "Reproduction of data processing script for rating studies (agency)"
date: last-modified
freeze: true
cache: true
format:
...
The is the script for processing of raw ratings data.

## Load packages

```{r}
#| label: original code lines 6-24 
...
```{r}
#| label: adaptation of code lines 28-41 to include {here} paths

files_conjoined <- list.files(path = here("R_scripts", "scripts_and_data_original", "Data_Preprocessing_original", "RatingsStudy", "prolific_data_conjoined"), pattern = "\\.csv$", full.names = T)

files_transitive <- list.files(path = here("R_scripts", "scripts_and_data_original", "Data_Preprocessing_original", "RatingsStudy", "prolific_data"), pattern = "\\.csv$", full.names = T)
...
# ELF: These data seem to correspond to the identification rating study:
length(unique(raw_data$subject)) # 128 participants
```

```{r}
#| label: adaptation of code lines 43-79 to include {here} paths

coding = read_csv(here("R_scripts", "scripts_and_data_original", "Data_Preprocessing_original", "RatingsStudy", "coding.csv")) #load file with regular/reverse coding info
...
test$first_mention_gender <- gsub('[[:digit:]]+', '', x=test$first_mention_gender) # Reproduction team: What did these digits represent?
...
## Data exploration by the reproduction team

We are curious to see how people actually rated the sentences. As we expected, it turns out that vast majority did not see any difference and picked the middle of the rating scale.
...
```{r}
#| label: original code lines 86-115
...
```{r}
#| label: original code lines 121-131 
...
```{r}
#| label: adapted code lines 136-228 
...
# Reproduction team: How many subjects would this exclude?
length(unique(repetitive3$subject)) # 45 people!
...
```{r}
#| label: original code lines 233-253
...
```{r}
#| label: original code lines 259-354 
...
# Reproduction team: The excluded participant did not report any gender:
demog |> 
  filter(subject == "73893")
...
```{r}
#| label: adapted code line 359 to add {here} path

write.csv(data, file = here("R_scripts", "scripts_reproduction", "Data_Preprocessing_reproduction", "reproduced_data","rating_study.csv"))

# ELF: This is the agency data for transitive and conjoint constructions.
```
