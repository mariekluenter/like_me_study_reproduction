---
title: "Reproduction of the overall memory data processing script"
date: last-modified
freeze: true
cache: true
format:
...
```{r}
#| label: Authors' comment at the top of the original script
#####
#
...
```{r}
#| label: Added {here} paths throughout as part of the reproduction effort
library(here)
```
...
```{r}
#| label: Reproduction team: Set working directory

setwd(here("R_Scripts", "scripts_and_data_original", "Data_Preprocessing_original", "Study1"))
```
...
# Replication team: Checking this figure of 19%:
nrow(data_bad) / nrow(test)*100
...
# Replication team: Mean is actually 0.56 but SD is correct.
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.2750  0.4750  0.5375  0.5565  0.6375  0.9875 
...
```{r}
setwd(here("R_Scripts", "scripts_and_data_original", "Data_Preprocessing_original", "Study2"))
...
```{r}
# Replication team: We are setting it to study 3 because that seems more logical - presumably a typo due to copy-and-pasting?

setwd(here("R_Scripts", "scripts_and_data_original", "Data_Preprocessing_original", "Study3"))
```
