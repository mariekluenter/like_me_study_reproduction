...
The data and code were made available on the OSF (<https://osf.io/87vew/overview>). The repository includes raw and analysis data, as well as cleaning and analysis code in R. As the analysed paper comprises a multitude of studies, the total number of subdirectories and files was somewhat overwhelming. The absence of a ReadMe file made navigating the data packages challenging. 

The preprocessing scripts for Study 1–3 (Process_Study1_Data.R, Process_Study2_Data.R, Process_Study3_Data.R) also mention that, due to a technical issue involving an unbacked-up computer, the authors no longer have access to all the initial raw data for these studies. However, they provide full access to data files, which have apparently only been hand-corrected for minor spelling mistakes (e.g. 'Rth' instead of 'Ruth'). We therefore classified these files as “raw data provided”. \
...
We were able to computationally reproduce the results of Studies 1-3 with the code provided by the authors. The only noticeable difference was a slight change in the numbers of the 95% bootstrapped confidence intervals with a maximal difference of ±0.2, which we believe is due to the way random seeds were set in the R scripts.

\
In Study 4, we were unable to fully reproduce two processed data files, study5_chinese.csv and data5_chinese.csv, from the raw data using the corresponding preprocessing script (Data Preprocessing/Chinese Study/Process_Chinese_Data.R). The script returned an error because the column \`TrialType\` does not exist. The raw data does include a column entitled \`trial_type\` which, however, only contains a single unique value: “survey_text”. By contrast, the \`Trial_Type\` variable contained in the processed data file uploaded by the authors,  data5_chinese.csv, has two levels, differentiating between “RaceTrial” and “GenderTrial”. We were unable to reproduce this variable on the basis of the raw data, but all other variables were successfully reproduced. We therefore had to use the study5_chinese.csv and data5_chinese.csv files provided by the authors to reproduce the statistical analyses based on these data. 

Moreover, we were unable to reproduce the median time taken to complete inputting the “critical sentence”, which according to the paper is 9.5 s in the English condition and 8.3 s in the Chinese condition (p.1713). It was not clear to us what exactly was meant by “critical sentence” and how these could be identified in the data sets. Furthermore, we obtained a minor difference in the final sample size for women in the English condition for Study 4. The paper refers to 28 women (p.1713), but after excluding those with slow RTs, the script “Process_Chinese_Data.R” contains a table with \`condition\`, \`ppt_gender\`, \`list\` and \`n\` in which only 27 English women are listed. 

We were able to reproduce the results of the ‘like me’ effect reported as part of Study 4. However, we were unable to locate the code necessary to reproduce some of the values reported in the paper and therefore had to write the code ourselves to check these numbers. This was the case for size of the overall ‘like me’ effect, reported as β = 0.18 (0.04), z = 5.0, P \< 0.001, which we successfully reproduced, as well as  the claim that “the size of the effect did not significantly differ depending on the language used” (p. 1710) where we reproduced again the same statistics as the authors apart from one value: we obtained a negative number for the z-value (z = -1.299) whereas, in the paper, a positive z-value (z = 1.3) is reported (p. 1710). This is likely to be a typo – it does not change the claims made in the paper. As for the earlier studies, we also observed minor differences in the values of the 95% bootstrapped confidence intervals. 

For the Empathy Rating Study, the preprocessing script (Data Preprocessing /EmpathyStudy/process_data.R) filtered participants but did not create the processed dataset used in subsequent analyses. Consequently, it was originally unclear to us whether the data used in the analysis script (Data Analysis/empathy_study_data.csv) matches the output of the preprocessing steps. However, we were eventually able to successfully recreate the dataframe used by the authors in their analysis, based on an adapted version of their preprocessing script, the information provided in the preregistration (<https://aspredicted.org/7698v.pdf>), and the raw data provided by the authors (Data Preprocessing /EmpathyStudy/full_empathy_data.csv).
...
For the Multi-Study analysis, which combines the data across Studies 1 through 3, the Methods section (p. 1712ff.), describes what is being investigated in that part of the paper: 
...
These regression models can be found in the provided code and match the descriptions in the paper. We successfully reproduced the results of these analyses. The same numbers can be reproduced by running the original code.
