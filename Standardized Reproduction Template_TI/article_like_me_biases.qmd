...
We were able to computationally reproduce the results of Studies 1-3 with the code provided by the authors. The only noticeable difference was a slight change in the numbers of the 95% bootstrapped confidence intervals with a maximal difference of ±0.2, which we believe is due to the way random seeds were set in the R scripts. <!--# ELF: If I remember right, this was because the authors set a seed at the top of a script and assumed it would apply to all randomised processes but, by default, set.seed() in R only applies to the next operation that involves randomisation. Please check and elaborate here to make clear why the seeds did not work as intended. -->
...
These regression models can be found in the code provided by the auhors ( <!--# ELF: Add file path plus name here -->) and match the descriptions in the paper. We successfully reproduced the results of these analyses.
